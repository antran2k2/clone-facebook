...
const FriendsShowing: React.FC<IMyProps> = props => {
  ...
  const friendsCopy = [...(props.userFriends?.friends || [])];
  ...
  return (
    <View style={styles.friendsWrapper}>
        ...
        <TouchableOpacity
          ...
          <View>
            ...
            <Text style={{fontSize: 16, fontWeight: '400', color: '#333'}}>
              {props.userFriends?.total} người bạn
              {isUserX && mututalCount > 0 ? `(${mututalCount} bạn chung)` : ''}
            </Text>
          </View>
          {!isUserX && (
            <TouchableOpacity
              ...
              <Text
                style={{fontSize: 16, color: '#318bfb', fontWeight: 'bold'}}>
                Tìm bạn bè
              </Text>
            </TouchableOpacity>
          )}
      ...
      <View style={styles.friendGallery}>
        {friendsCopy.splice(0, 6).map((friend, index) => (
