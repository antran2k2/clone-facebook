...
const ConfirmScreen = () => {
  const [checkVerifyCode, {isLoading}] = useCheckVerifyCodeMutation();
  const [loginMutate, {isLoading: isLoadingLogin}] = useLoginMutation();
  const [getVerifyCode, {isLoading: isLoadingGetCode}] =
    useGetVerifyCodeMutation();
  ...
  const handleConfirm = () => {
    setAlertText('');
    checkVerifyCode({email, code_verify: code})
      .unwrap()
      .then(() => {
        loginMutate({email, password, uuid: password})
          .unwrap()
          .then(() => {
  ...
  return (
    <View style={styles.container}>
      <Spinner visible={isLoading || isLoadingLogin || isLoadingGetCode} />
