...
const FriendRequestScreen = () => {
  ...
  const [acceptFriend, {isLoading: isLoadingAccept}] =
    useSetAcceptFriendMutation();
  const [delRequestFriend, {isLoading: isLoadingDel}] =
    useDelRequestFriendMutation();
  ...
  const [friendRequests, setFriendRequests] = useState(data?.requests);
  const [friendSelect, setFriendSelect] = useState<TFriend>();
  const [arrayFriendAccept, setArrayFriendAccept] = useState<
    Array<String | undefined>
  >([]);
  const [arrayFriendRemove, setArrayFriendRemove] = useState<
    Array<String | undefined>
  >([]);
  ...
  const handleRemoveFriend = () => {
    acceptFriend({user_id: friendSelect?.id || '-1', is_accept: '0'})
      .unwrap()
      .then(res => {
    ...
    <View style={styles.container}>
      <View style={styles.navigationBar}>
        ...
        <View style={styles.navigationBarBottom}>
          ...
          <TouchableOpacity
            style={styles.navigation}
            onPress={onPressFullFriendHandler}>
            <Text style={{fontSize: 16, fontWeight: '500', color: '#000'}}>
              Tất cả bạn bè
            </Text>
          </TouchableOpacity>
        ...
        <View
          ...
          <View>
            <Text
              ...
              Những người gửi cho bạn lời mời kết bạn sẽ xuất hiện ở đây
            </Text>
          </View>
          <TouchableOpacity
            style={{
          ...
          <View style={styles.friendRequestsWrapper}>
            <View style={styles.friendRequestsWrapperHeader}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.5}>
                <Text style={{fontSize: 18, color: '#318bfb'}}>Sắp xếp</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.friendRequests}>
              {friendRequests?.length > 0 &&
                friendRequests.map((friendRequest, index) => (
                  <TouchableOpacity
                    onPress={onPressProfileHandler.bind(
                      this,
                      friendRequest?.id,
                    )}
                    key={index}
                    ...
                    <View style={styles.recommendInfo}>
                      <View>
                        ...
                        {!arrayFriendAccept.includes(friendRequest?.id) &&
                          !arrayFriendRemove.includes(friendRequest?.id) &&
                        ...
                        {arrayFriendAccept.includes(friendRequest?.id) ? (
                          <View>
                            <Text style={styles.statusText}>
                              Các bạn hiện là bạn bè của nhau
                            </Text>
                          </View>
                        ) : arrayFriendRemove.includes(friendRequest?.id) ? (
                          <View>
                          ...
                          <View style={styles.btnActionsWrapper}>
                            <TouchableOpacity
                              ...
                              onPress={onPressAcceptsHandler.bind(
                                this,
                                friendRequest,
                              )}>
                              <Text
      ...
      <Modal
        ...
        <ArrangeWrapper
          sortDataByCreateAt={sortDataByCreateAt}
        ...
        <View style={styles.postOptionsWrapper}>
          ...
          <View style={styles.postOptionItemWrapper}>
            <Text
              ...
              Bạn có chắc chắn muốn kết bạn với {friendSelect?.username} không ?{' '}
            </Text>
          </View>
        ...
        <View style={styles.postOptionsWrapper}>
          <View style={styles.postOptionItemWrapper}>
            <Text
              ...
              Gỡ lời mời từ {friendSelect?.username}
            </Text>
          </View>
          <View style={styles.postOptionItemWrapper}>
            <Text
              ...
              Bạn có chắc chắn muốn gỡ lời mời kết bạn từ{' '}
              {friendSelect?.username} không ?
            </Text>
          </View>
          <View style={styles.postOptionWrapperEnd}>
            ...
            <View style={styles.postOptionItem}>
              ...
              <View>
                ...
                <Text style={{fontSize: 16, paddingRight: 30}}>
                  {friendSelect?.username} sẽ không nhận được thông báo
                </Text>
              </View>
            </View>
          </TouchableOpacity>
            ...
            <View style={styles.postOptionItem}>
              ...
              <View>
                <Text
                  ...
                  Chặn {friendSelect?.username}
                </Text>
                <Text style={{fontSize: 16, paddingRight: 30}}>
                  {friendSelect?.username} sẽ không thể nhìn thấy bạn hay liên
                  hệ với bạn trên AntiFacebook
                </Text>
              </View>
            </View>
          </TouchableOpacity>
